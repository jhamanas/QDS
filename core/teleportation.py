...
Purpose
-------
Implements the standard 3-qubit teleportation circuit: Aditi teleports an
unknown single-qubit state to Bharat using one shared Bell pair and two
...
Register layout (matching the MSB-first convention from primitives.py):
    qubit 2 : "A"  -- the message qubit Aditi wants to teleport
    qubit 1 : "B"  -- Aditi's half of the shared Bell pair
    qubit 0 : "C"  -- Bharat's half of the shared Bell pair (this is the
...
Circuit steps:
    ...
    3. Aditi applies CNOT(control=A, target=B).
    4. Aditi applies H to A.
    5. Aditi measures A and B in the Z (computational) basis, obtaining
       two classical bits (m_A, m_B).
    6. Aditi sends (m_A, m_B) to Bharat over a classical channel.
    7. Bharat applies a Pauli correction to C based on the bits received:
       specifically Z^{m_A} X^{m_B} (see NOTE below on why this order).
    8. Bharat's qubit C now holds the original message state exactly.

NOTE on correction order: teleportation's standard derivation shows the
by-product operator left on Bharat's qubit (before correction) is
...
N_QUBITS = 3
MSG_QUBIT = 2     # Aditi's message qubit "A"
ADITI_BELL_QUBIT = 1   # Aditi's half of the Bell pair "B"
BHARAT_BELL_QUBIT = 0     # Bharat's half of the Bell pair "C"
...
def teleport_qubit(psi: np.ndarray, rng: np.random.Generator,
    ...
    Parameters
    ----------
    psi : the 2-element statevector Aditi wants to teleport to Bharat.
    ...
    -------
    dict with:
        "received_state"      -- Bharat's qubit after correction (should
                                  equal `psi` with fidelity 1.0 in the
                                  honest, noiseless case)
        "pre_correction_state"-- Bharat's qubit BEFORE correction (useful
                                  for later attack/detection analysis)
        "classical_bits"      -- (m_A, m_B) the classical bits Aditi sends
    ...
    full_state = apply_two_qubit_gate(
        full_state, CNOT, qubits=(MSG_QUBIT, ADITI_BELL_QUBIT), n_qubits=N_QUBITS
    )
    ...
    m_B, full_state = measure_qubit(full_state, target=ADITI_BELL_QUBIT,
                                     n_qubits=N_QUBITS, rng=rng)

    # Extract Bharat's now-definite single-qubit state (see extract_reduced_state
    # docstring for why this is valid: only one qubit remains unmeasured).
    pre_correction = extract_reduced_state(
        full_state, target_qubit=BHARAT_BELL_QUBIT,
        fixed={MSG_QUBIT: m_A, ADITI_BELL_QUBIT: m_B}, n_qubits=N_QUBITS
    )
...
def required_correction(m_A: int, m_B: int) -> str:
    """
    Human-readable description of which correction Bharat applies for a
